...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-black fixed z-[999]">
      <div>
      ...
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          ...
          >
            <Link to={link} smooth={true} spy={true} duration={500}>
              {link}
            </Link>
          </li>
        ))}
      ...
      {nav && (
        ...
        >
          {links.map(({ id, link }) => (
            ...
            >
              <Link onClick={() => setNav(!nav)} to={link} smooth={true} spy={true} duration={500}>
              {link}
            </Link>
            </li>
          ))}
